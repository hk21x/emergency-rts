...
func setup(u: UnitDef, qty: int, can_add: bool) -> void:
	...
	# And a ceiling as well as a floor, so nothing inside a row can push it taller.
	...
	g.texture = u.icon
	# **The same `EXPAND_KEEP_SIZE` trap as [UnitCard], and here it grew the whole modal.**
	# A TextureRect's minimum size is its texture's size unless told otherwise, so each
	# 192x192 portrait made a row ~192px tall against the 52 intended -- and the order
	# panel is what the modal sizes itself around, so adding a third unit to the cart
	# stretched the dialog to the full height of the screen.
